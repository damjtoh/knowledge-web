...
- Compose the desktop shell from registry-installed sidebar-11 as the required starting point: use the generated SidebarProvider/Sidebar/SidebarHeader/SidebarContent/SidebarInset/SidebarTrigger and SidebarMenu/Collapsible tree composition as the actual shell and file tree. Use shadcn registry components exclusively for shell controls. Preserve bespoke article typography and needed responsive styling. Do not hand-write a CSS sidebar or drawer primitive. Preserve a bounded article width. Collapse the desktop sidebar fully (offcanvas) rather than showing a file-icon rail.
...
- Preserve grayscale light/dark styling, high-contrast active and focus states, readable long labels, and adequate touch targets. Prefer the project's current styling and component conventions; adopt only the UI primitives needed for this shell. Install sidebar-11 and required UI components via the official shadcn registry CLI and keep installation evidence; do not copy block sample data as product data.
...
- The registry-installed sidebar-11 block is the required shell and file-tree starting point, installed via the official shadcn registry CLI. Its sample Changes/Files data is a layout reference only and must not ship as product data. In particular, the sample folder row's single expand action must not replace this reader's separate folder-page link.
